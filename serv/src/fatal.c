...
const char* get_register_name(int reg_id) {
...
void print_all_registers() {
    printf("x86 32位通用寄存器列表:\n");
    printf("========================\n");
    
    for (int i = 0; i < 10; i++) {
        printf("寄存器 %d: %s\n", i, get_register_name(i));
    }
}
#if 0
int main() {
    ...
    // 测试获取特定寄存器名称
    printf("寄存器ID 3 对应的名称: %s\n", get_register_name(3));
    printf("寄存器ID 7 对应的名称: %s\n\n", get_register_name(7));
    ...
    printf("请输入寄存器ID (0-9): ");
    if (scanf("%d", &reg_id) == 1) {
        printf("寄存器 %d 的名称是: %s\n", reg_id, get_register_name(reg_id));
    }
